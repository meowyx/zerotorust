...
export type Episode = {
  ...
  pdf: string | null;
  // Teaching deck: a present-in-class slide view served from the same folder.
  // null hides the button (most episodes have no dedicated deck yet).
  slides: string | null;
};
...
type EpisodeSeed = Omit<Episode, "youtube" | "pdf" | "slides">;
...
const series1: Episode[] = SERIES_1_SEED.map((e) => ({
  ...
  // Every companion guide also ships a matching slide deck (generated by
  // scripts/build-slides.ts, except 01 which is hand-tuned), so slides track
  // the guide from the same map, no separate list to drift out of sync.
  slides: NOTES_SLUG[e.num] ? `/notes/${NOTES_SLUG[e.num]}/slides.html` : null,
}));
